    ...
    <div class="table_center">
      <table class="content">
        ...
        <tr style="height: 40px">
         ...
         <tr style="height: 40px">
          <td colspan="1" width="200">荣誉名称</td>
          <td colspan="5">
            <el-input
              v-model="form.honor"
              placeholder="请输入"
...
<script>
import { honorEditInit,honorEditSubmit,getAuthHeader, getPersonImage } from "@/service/genServ.js";
export default {
  name: "HonorEdit",
  data() {
    return {
  ...
  methods: {
    fetchData() {
      honorEditInit({ id: this.id }).then((res) => {
    ...
    doSumit() {
      honorEditSubmit({ id: this.id, form: this.form }).then(
        (res) => {
          if (res.code === 0) {
            this.$message({
              message: '保存成功！',
